...
for route in / /produtos /movimentacoes /historico; do curl -fsS "$BASE_URL$route" >/dev/null; done
...
code=$(curl -sS -o /tmp/etapa9-e2e-erro.json -w '%{http_code}' -X POST "$BASE_URL/api/movimentacoes" -H 'Content-Type: application/json' -d '{"sku":"E2E-001","tipo":"SAIDA","quantidade":100}')
[[ "$code" == "400" ]]
...
if grep -q 'E2E-002' <<<"$filtered"; then echo 'Falha: filtro de histórico misturou E2E-002' >&2; exit 1; fi

python - "$BASE_URL" <<'PY'
import json, sys, urllib.request
base=sys.argv[1]
with urllib.request.urlopen(base + '/api/produtos') as r: products=json.load(r)
